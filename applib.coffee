...
if path.existsSync 'myport'
  process.config.port = fs.readFileSync 'myport', 'utf8'
...
build = (toload, which) ->
  ...
  "<!doctype html><html><head><meta charset=\"utf-8\"><meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\"><title>OIC</title>#{css}</head><body>#{idthis}#{body}</body></html>"
