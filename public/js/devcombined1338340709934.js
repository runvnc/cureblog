...
(function() {
  ...
  EmailCollectorWidget = (function() {
    ...
    function EmailCollectorWidget(position, exists, widget) {
      var pages, pageshtml;
      console.log('position.top is ' + position.top);
      console.log('position.left is ' + position.left);
      if (!exists) {
        pageshtml = $('#emailswidgettemplate').html();
        pages = $(pageshtml);
        pages.css('position', 'absolute');
        pages.css('top', position.top + 'px');
        pages.css('left', position.left + 'px');
    ...
    function EmailCollectorTool() {
      ...
      widget.draggable({
        ...
        stop: function(ev, ui) {
          ...
          return new EmailCollectorWidget(p, false);
        }
      });
    }
  ...
  $(function() {
    $('.emailsall').each(function() {
      ...
      if ($(this) != null) {
        x = $(this).position().left;
        y = $(this).position().top;
        return text = new EmailCollectorWidget($(this).position(), true, $(this));
      }
    });
    return $(document).bind('sessionState', function(user) {
      if (window.loggedIn) {
